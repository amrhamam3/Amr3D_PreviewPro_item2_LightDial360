 ...
 * الـ UI thread)، الإبطاء ده مقبول مقابل الدقة — لازم اختبار حقيقي على موديل
 * كبير فعليًا (مئات الآلاف من المثلثات) للتأكد من وقت الاستجابة على جهاز حقيقي.
 */
object EdgeCollapseDecimator {
    ...
    fun run(model: STLModel, keepRatio: Float): STLModel? {
        ...
        val vertTris = Array(vertexCount) { HashSet<Int>() }
        for (t in 0 until triangleCount) {
